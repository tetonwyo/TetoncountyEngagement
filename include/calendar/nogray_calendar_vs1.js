...
Options:
	...
	formmater: a function that will return the HTML for the table cell.
				arguments JavaScript Date
				default returns the date
				
	outOfRangeFormmater: a function that will return the HTML for out of range date table cell.
				arguments JavaScript Date
				default returns the date
				
	weekendFormmater: a function that will return the HTML for the weekend table cell.
				arguments JavaScript Date
				default returns the date
				
	
	daysOffFormmater: a function that will return the HTML for the days off table cell.
				arguments JavaScript Date
				default returns the date
				
	datesOffFormmater: a function that will return the HTML for dates off table cell.
				arguments JavaScript Date
				default returns the date
				
	selectedDateFormmater: a function that will return the HTML for the selected date table cell.
				arguments JavaScript Date
				default returns the date
...
var Calendar = new Class({
	options:{
		...
		formmater: function(dt){
			return dt.getDate();
		},
		outOfRangeFormmater: function(dt){
			return dt.getDate();
		},
		weekendFormmater: function(dt){
			return dt.getDate();
		},
		daysOffFormmater: function(dt){
			return dt.getDate();
		},
		datesOffFormmater: function(dt){
			return dt.getDate();
		},
		selectedDateFormmater:function(dt){
			return dt.getDate();
		},
	...
	initialize: function(el, toggler, options){
		...
		this.options.tdEvents = options.tdEvents;
			...
			t[t.length] = '<tr>';
				for (j=1; j<=7; j++){
					...
					if (date.getMonth() != mn){
						...
						t[t.length] = '<td class="'+this.options.classPrefix+'date-'+id_str+' '+this.options.classPrefix+'outOfRange'+'">'+this.options.outOfRangeFormmater(date)+'</td>';
					}
					else {
	...
	selectDate: function(date){
		...
		if (this.isSelectable(date)){
			...
			if ($defined($(id_str))){
				...
				$(id_str).setHTML(this.options.selectedDateFormmater(this.options.selectedDate));
			}
